...
import model.Sheet424CorePDM;
import model.Sheet424PersonalPDM;
...
public class Sheet424Filler {
    ...
    private static void insertTimeStatusPair(Row row
        ...
        for(int i = 0; i < data.size(); i++){
            ...
            cellTime.setCellValue(ExcelUtils.timestampToString(time));

            Cell cellStatus = ExcelUtils.getCell(row
                    , ExcelConfig.Sheet424Start + 2*i + 1);
    ...
    private static void insert(Sheet sheet424
        ...
        int index = sheet424.getLastRowNum() + 1;
